...
function NodePropertiesPanel({
    ...
    const [isFetchingWhatsappChats, setIsFetchingWhatsappChats] = useState(false);
    ...
    const [educationData, setEducationData] = useState<any>({ classes: [], subjects: [], assignments: [], students: [] });
    ...
    const fetchRecentResponses = useCallback(async (formId: string) => {
        if (!formId || formId.includes('/d/')) return; // Skip if it looks like a URL still
    ...
    const fetchEducationData = useCallback(async () => {
        ...
        try {
            const res = await fetch('/api/education/data');
            const data = await res.json();
            if (data.ok) {
    ...
    const fetchWhatsappChats = useCallback(async () => {
        setIsFetchingWhatsappChats(true);
        try {
            ...
            setIsFetchingWhatsappChats(false);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if ((node.data.nodeType === 'google-forms' || node.data.nodeType === 'trigger-form') && config.formId) {
            fetchRecentResponses(config.formId);
        }

        const educationNodes = ['attendance-track', 'assignment-create', 'schedule-check', 'grade-calculate', 'student-notify', 'quiz-create', 'report-generate'];
        if (educationNodes.includes(node.data.nodeType)) {
            fetchEducationData();
        }

        if (node.data.nodeType === 'whatsapp-group' || node.data.nodeType === 'alert-send' || node.data.nodeType === 'twilio-whatsapp') {
            fetchWhatsappStatus();
            fetchWhatsappChats();
        }
    }, [node.data.nodeType, fetchClassroomMetadata, fetchRecentSheets, fetchRecentDriveFiles, fetchRecentCalendars, fetchRecentEvents, fetchRecentForms, fetchRecentResponses, config.calendarId, config.formId, fetchEducationData, fetchWhatsappStatus, fetchWhatsappChats]);
    ...
    const renderConfigFields = () => {
        switch (node.data.nodeType) {
            case "whatsapp-group":
                ...
                return (
                    <>
                        {/* MODE Selection - Always WhatsApp Web Now */}
                        <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg mb-4 border border-green-200 dark:border-green-800">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-medium text-green-700 dark:text-green-400">WhatsApp Web Messenger</span>
                            </div>
                            <p className="text-xs text-green-600 dark:text-green-500">
                                ✅ Messages will be sent directly via your connected WhatsApp account.
                            </p>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                                <Label htmlFor="whatsappChatId" className="text-green-700 dark:text-green-400 font-semibold">
                                    📱 Select Recipient (Contact or Group)
                                </Label>
                                <div className="flex gap-2">
                                    <Select
                                        value={config.whatsappChatId || ""}
                                        onValueChange={(val) => handleConfigChange("whatsappChatId", val)}
                                    >
                                        <SelectTrigger className="flex-1 border-green-300 dark:border-green-700 bg-white dark:bg-black/20">
                                            <SelectValue placeholder="Select a contact or group..." />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {whatsappChats.length === 0 && (
                                                <div className="p-2 text-xs text-muted-foreground text-center">
                                                    No chats loaded. Click refresh button.
                                                </div>
                                            )}
                                            {whatsappChats.map((c: any) => (
                                    ...
                                    <Button
                                        variant="outline"
                                        size="icon"
                                        onClick={fetchWhatsappChats}
                                        disabled={isFetchingWhatsappChats}
                                        ...
                                        title="Fetch Chats"
                                    >
                                        <RefreshCw className={`w-4 h-4 ${isFetchingWhatsappChats ? 'animate-spin' : ''}`} />
                                    </Button>
                                </div>
                                <p className="text-xs text-green-600 dark:text-green-400">
                                    Select the contact or group from the list. Make sure you are connected in Integrations.
                                </p>
                            </div>
                        </div>
            ...
            case "alert-send":
                return (
                    <>
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="recipients">Manual Recipients (Phone Numbers/Emails)</Label>
                            <Textarea
                                id="recipients"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="whatsappGroupId">Target WhatsApp Contact/Group (Optional)</Label>
                            <div className="flex gap-2">
                                <Select
                                    value={config.whatsappGroupId || "none"}
                                    onValueChange={(value) => handleConfigChange("whatsappGroupId", value)}
                                >
                                    <SelectTrigger className="flex-1">
                                        <SelectValue placeholder="Select a chat..." />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="none">None (Use Manual Only)</SelectItem>
                                        {whatsappChats.map((c: any) => (
                                            <SelectItem key={c.id} value={c.id}>
                                                <span className="flex items-center gap-2">
                                                    {c.isGroup ? <Users className="w-3 h-3" /> : <User className="w-3 h-3" />}
                            ...
                            <p className="text-[10px] text-muted-foreground italic">
                                If selected, the alert will also be sent to this WhatsApp contact/group.
                            </p>
                        </div>
                        <div className="space-y-2">
            ...
            case "reminder":
                return (
                    <>
                        <div className="p-4 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-lg mb-4 border border-amber-200 dark:border-amber-800">
                            <div className="flex items-center gap-2 mb-2">
                                <Bell className="w-5 h-5 text-amber-600" />
                                <span className="font-medium text-amber-700 dark:text-amber-400">Reminder Node</span>
                            </div>
                            <p className="text-xs text-amber-600 dark:text-amber-500">
                                🔔 Schedule and send reminders via WhatsApp or Email.
                            </p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                id="reminderTitle"
                                placeholder="Assignment Due Date Reminder"
                                value={config.reminderTitle || ""}
                                onChange={(e) => handleConfigChange("reminderTitle", e.target.value)}
                            />
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea
                                id="reminderMessage"
                                placeholder="Don't forget to submit your assignment by tomorrow at 5 PM!"
                                value={config.reminderMessage || ""}
                                onChange={(e) => handleConfigChange("reminderMessage", e.target.value)}
                                rows={4}
                            />
                        </div>
                        ...
                        {config.channel === "whatsapp" && (
                            <div className="space-y-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                                <Label htmlFor="whatsappChatId" className="text-green-700 dark:text-green-400">
                                    📱 Select Recipient
                                </Label>
                                <div className="flex gap-2">
                                    <Select
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-muted-foreground">Wait this many minutes before sending the reminder (optional)</p>
                        </div>
            ...
            case "attendance-track":
                return (
                    <>
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="classId">Class Selection</Label>
                            <div className="flex gap-2">
                                <Select
                                    value={config.classId || "all"}
                                    onValueChange={(value) => handleConfigChange("classId", value)}
                                >
                                    <SelectTrigger className="flex-1">
                                    ...
                                    <SelectContent>
                                        ...
                                        {educationData.classes.map((c: string) => (
                                ...
                                >
                                    <RefreshCw className={`w-4 h-4 ${isFetchingEducation ? 'animate-spin' : ''}`} />
                                </Button>
                            </div>
                            {educationData.classes.length === 0 && !isFetchingEducation && (
                                <p className="text-[10px] text-muted-foreground italic mt-1">
                                    No classes found in your schedule. Data is synced from your dashboard.
                                </p>
                            )}
                        </div>
                        <div className="space-y-2">
                    ...
                    <>
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    {educationData.subjects.map((s: string) => (
                ...
                return (
                    <>
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="subject">Filter by Subject (Optional)</Label>
                                <Select
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="classId">Filter by Class (Optional)</Label>
                                <Select
                                    value={config.classId || "all"}
                                    onValueChange={(value) => handleConfigChange("classId", value)}
                                >
            ...
            case "grade-calculate":
                return (
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="assignmentId">Reference Assignment</Label>
                            <Select
                                value={config.assignmentId || ""}
                                onValueChange={(value) => handleConfigChange("assignmentId", value)}
                            >
                                <SelectTrigger>
                                    <SelectValue placeholder="Select Assignment" />
                                </SelectTrigger>
                                <SelectContent>
                                    {educationData.assignments.map((a: any) => (
