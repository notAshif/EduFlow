...
export class LocalAINode extends BaseNode {
  validate(config: any): void {
    ...
    if (!['summary', 'keywords', 'feedback', 'ollama'].includes(config.mode)) {
      throw new Error('Invalid mode. Must be summary, keywords, feedback, or ollama');
    }
  }

  async execute(context: NodeExecutionContext): Promise<any> {
    const { text, mode, maxLength = 100, model = "llama3", baseUrl = "http://localhost:11434" } = this.config;
    const input = context.input?.text || context.input?.data || text;

    try {
      switch (mode) {
        case 'ollama':
          return this.executeOllama(input, model, baseUrl);
        case 'summary':
          return this.generateSummary(input, maxLength);
        case 'keywords':
          return this.extractKeywords(input);
        case 'feedback':
      ...
      // Fallback to rule-based summary if Ollama fails and it's important
      if (mode === 'ollama') {
        console.warn('[LocalAI] Ollama failed, falling back to rule-based summary');
        return this.generateSummary(input, maxLength);
      }
  ...
  private async executeOllama(text: string, model: string, baseUrl: string): Promise<any> {
    const result = await LLMService.chat(
      "You are a local AI assistant. Process the following text and provide a concise response.",
      text,
      {
        provider: 'ollama',
        model,
